...
**하루메일 / HaruMail**은 한국어·영어·일본어·스페인어·포르투갈어·프랑스어·독일어로 제공하는 무료 임시메일 서비스입니다. 주소 생성·복사와 받은편지함을 중심으로 한 메일 앱이며, 모바일에서 목록과 본문을 전환할 수 있습니다. 연결 재시도, 수동 복사, 세션 만료 시 표시 내용 정리와 키보드 포커스 복귀를 지원합니다.
...
구현된 항목:
- 7개 언어별 가이드 5편, FAQ, 소개, 문의, 개인정보처리방침, 이용약관 (공개 페이지 총 84개)
...
- 한국어 `/`, 영어 `/en/`, 일본어 `/ja/`, 스페인어 `/es/`, 포르투갈어 `/pt/`, 프랑스어 `/fr/`, 독일어 `/de/`.
- `content/locales/ui.tsv`는 한국어 UI 원문과 6개 번역, `content/locales/content.tsv`는 콘텐츠 키와 6개 번역을 저장합니다. 구분자는 `|`이며 열 순서는 en/ja/es/pt/fr/de입니다. UI의 앞뒤 공백은 의미가 있으므로 보존하세요.
- `scripts/i18n.js`가 정적 페이지와 공통 앱의 언어별 빌드를 만듭니다. 누락된 번역은 빌드를 실패시킵니다. 새 기능의 한국어 문자열을 추가하면 6개 번역과 동작 검증을 함께 추가하세요.
- 언어별 canonical과 상호 hreflang, 영어 x-default, 크롤링 가능한 언어 링크와 sitemap을 제공합니다. API 주소와 세션 쿠키는 언어와 무관하므로 전환해도 같은 메일함을 유지합니다.
- 조사 근거와 적용 범위: `docs/2026-09-07-international-seo-research.md`. 검색량을 확보하지 않았으며 순위와 색인은 보장하지 않습니다.
- 다국어 페이지는 광고 스크립트를 로드하지 않습니다. 광고를 확대하기 전에 각 언어의 안내와 동의 관리를 실제 설정과 일치시켜야 합니다.
