...
function issueMailbox(now = Date.now()) {
  ...
  const mailbox = { address: `${randomBytes(16).toString('hex')}@${domain.toLowerCase()}`, createdAt: now, expiresAt: now + TTL };
...
function verifyToken(token, now = Date.now()) {
  ...
  if (!/^[a-f0-9]{32}@[a-z0-9.-]+$/.test(mailbox.address || '') || !Number.isFinite(mailbox.createdAt) || !Number.isFinite(mailbox.expiresAt) || mailbox.expiresAt - mailbox.createdAt !== TTL || mailbox.createdAt > now + 5000 || mailbox.expiresAt <= now) throw failure('메일함 이용기간이 끝났습니다. 새 주소를 만들어 주세요.');
  return mailbox;
}
