...
function catalogs(root){
 ...
 for(const filename of ['ui.tsv','content.tsv'])for(const line of fs.readFileSync(path.join(root,'content/locales',filename),'utf8').split(/\r?\n/).filter(Boolean)){
...
function buildLocales({root,dist,config,escape,verification,koreanRoutes}){
 ...
 for(const [lang,,intl,og] of languages.slice(1)){
  ...
  for(const [slug,key,next] of guideSpecs){
   ...
   const content=`<nav class="breadcrumb" aria-label="${e('홈')}">${crumbs.map(c=>`<a href="${c.path}">${escape(c.name)}</a>`).join(' / ')}</nav><p class="article-meta">HaruMail · ${e('updated')}</p>${p(key+'Body')}${p(next)}<aside class="related-guides"><h2>${e('관련 가이드')}</h2><ul>${related}</ul></aside><p><a href="${url('/')}">${e('무료 주소 만들기')}</a></p>`;
  ...
  const info={about:['서비스 소개',p('aboutBody')+p('introBody')+p('limitsBody')],contact:['문의',p('contactBody')+(config.supportEmail?`<p><a href="mailto:${escape(config.supportEmail)}">${escape(config.supportEmail)}</a></p>`:p('contactPending'))],privacy:['개인정보처리방침',`<p>${e('updated')}</p>`+['Data','Retention','Providers','Ads'].map(k=>`<h2${k==='Retention'?' id="retention"':''}>${e('privacy'+k+'Title')}</h2>${p('privacy'+k+'Body')}`).join('')+`<p><a href="https://www.cloudflare.com/privacypolicy/">Cloudflare</a> · <a href="https://supabase.com/privacy">Supabase</a> · <a href="https://vercel.com/legal/privacy-policy">Vercel</a></p>`],terms:['이용약관',`<p>${e('updated')}</p>`+p('introBody')+p('limitsBody')+p('termsBody')+p('termsLimits')]};
