...
function buildLocales({root,dist,config,escape,verificationMarkup,koreanRoutes}){
 ...
 for(const [lang,,intl,og] of languages.slice(1)){
  ...
  const info={about:['서비스 소개',p('aboutBody')+p('introBody')+p('limitsBody')+`<h2>${e('editorialHeading')}</h2>`+p('editorialBody')+(config.operatorName?`<p>${e('operatorLabel')}: ${escape(config.operatorName)}</p>`:'')],contact:['문의',p('contactBody')+p('contactDetails')+(config.operatorName?`<p>${e('operatorLabel')}: ${escape(config.operatorName)}</p>`:'')+(config.supportEmail?`<p><a href="mailto:${escape(config.supportEmail)}">${escape(config.supportEmail)}</a></p>`:p('contactPending'))],privacy:['개인정보처리방침',`<p>${e('updated')}</p>`+['Data','Retention','Providers','Ads'].map(k=>`<h2${k==='Retention'?' id="retention"':''}>${e('privacy'+k+'Title')}</h2>${p('privacy'+k+'Body')}${k==='Data'?p('cookieDetails'):''}`).join('')+`<p><a href="https://www.cloudflare.com/privacypolicy/">Cloudflare</a> · <a href="https://supabase.com/privacy">Supabase</a> · <a href="https://vercel.com/legal/privacy-policy">Vercel</a></p>`],terms:['이용약관',`<p>${e('updated')}</p>`+p('introBody')+p('limitsBody')+p('termsBody')+p('termsLimits')]};
